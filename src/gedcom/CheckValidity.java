			...
			if (familyRecord.getMarriageDate() != null) {
				...
				for (String childId : childrenList) {
					...
					if(!check_father_not_too_old(husbandAge, childAge)) {
						...
						error.setErrorDetails("father " + husbandId + " " + husbandName + "is more than 80 years older than his children" + childId + " " + childName);
						errorList.add(error);
					}
					if(!check_mother_not_too_old(wifeAge, childAge)) {
						...
						error.setIndividualId(familyId);
						error.setUserStoryNumber("US12");
						error.setErrorDetails("mother " + wifeId + " " + wifeName + "is more than 60 years older than his child" + childId + " " + childName);
						errorList.add(error);
					}
					//US17
					if(!check_father_no_marriages_to_children(wifeId, childId)) {
						...
						error.setErrorDetails("father " + husbandId + " " + husbandName + "married with his child" + childId + " " + childName);
						errorList.add(error);
					}
					if(!check_mother_no_marriages_to_children(husbandId, childId)) {
						...
						error.setIndividualId(familyId);
						error.setUserStoryNumber("US17");
						error.setErrorDetails("mother " + wifeId + " " + wifeName + "married with her child" + childId + " " + childName);
						errorList.add(error);
					}
				}
